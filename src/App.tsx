...
function App() {
  ...
  useEffect(() => {
    if (activeTab === "All") {
      const allItems =
        query?.data?.categories?.flatMap((category: any) => category.items) ||
        [];
      setItems(allItems);
    }
    ...
    <>
      <div className="md:px-72 px-2 py-10   w-full h-screen overflow-hidden ">
        ...
        <div className="border md:p-10">
          ...
          {/* menu */}
          <div className="space-y-4 mt-10 overflow-y-scroll ">
            {filteredItems?.map((item: any) => {
              return (
                <div
                  key={item.id}
                  className="flex p-5 border rounded-lg hover:shadow-lg mx-2"
                >
